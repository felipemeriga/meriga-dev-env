# Tide prompt configuration — captured from a working setup.
# Sourced by install.sh (followed by "tide reload") to reproduce the prompt
# on a fresh machine, since tide config normally lives in gitignored fish_variables.
# Regenerate with: see comment in install.sh / README.
